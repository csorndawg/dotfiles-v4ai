...
# cd to tmux.d/ set as cwd
echo cd "$TMUX_SYML_SRC"
cd "$TMUX_SYML_SRC"
echo "CWD: $(pwd -P)"

ln -sfr "$TMUX_SYML_SRC" "$TMUX_CONF_DIR" || echo "Issue creating symlink between dotfiles tmux and local config"
...
# remove self-referencing symlink duplicate
